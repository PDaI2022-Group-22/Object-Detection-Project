...
class Measurement:
    ...
    def __init__(self, widthM, heightM, addressM):
        self.widthM = widthM
        self.heightM = heightM
        self.addressM = addressM
        print("hello")
        
        
    def capture(self):
        cap = cv2.VideoCapture(self.addressM)
        print("hello")
        while(True):
            ret, frame = cap.read()
            out = cv2.transpose(frame)
            out = cv2.flip(out,flipCode=1)
            imgResized = utils.resize(out,width=600,height=self.imgHeightPrefix)
